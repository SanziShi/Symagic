...
CREATE  TABLE IF NOT EXISTS `bookshop`.`BOOK` (
  ...
  `FOLIO` VARCHAR(2) NOT NULL DEFAULT '2' COMMENT '书的开本，默认为2开' ,
...
CREATE  TABLE IF NOT EXISTS `bookshop`.`FAVORITY_FOLDER` (
  ...
  `USERID` INT UNSIGNED NOT NULL COMMENT '用户ID,外链到user表的USERID' ,
...
CREATE  TABLE IF NOT EXISTS `bookshop`.`BOOK_ORDER` (
  ...
  `USERID` INT UNSIGNED NOT NULL COMMENT '用户id关联到用户表的userid.' ,
...
CREATE  TABLE IF NOT EXISTS `bookshop`.`ORDER_DETAIL` (
  ...
  `DISCOUNT` TINYINT NOT NULL DEFAULT 100 COMMENT '折扣率，实际价格要通过MARKETPRICE*DISCOUNT得到。' ,
