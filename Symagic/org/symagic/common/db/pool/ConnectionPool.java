...
public class ConnectionPool {
	...
	static Stack<Connection> st = new Stack<Connection>(); // 创建一个堆栈，作为池子
	private static ConnectionPool instance = null; // 单例模式
	...
	/**
	 * 说明：连接池私有构造方法，用于单例模式。 功能：初始化连接池
	 */
	private ConnectionPool() {

		// String path =
		// this.getClass().getClassLoader().getResource("jdbc.json").getPath();

		String path = FileUtils.getUserDirectoryPath() + "/jdbc.json";

		// String path = "jdbc.json";

		String sets = Util.readFile(path);
		try {
			java.util.HashMap<String, String> map = (java.util.HashMap<String, String>) JSONUtil
					.deserialize(sets);
